...
const CONFIG = {
    ...
    TEMPLATES: {
        ...
        CLIENT_CITY: 'HX232d288f7201dcedae6c483b80692b9d', // הטמפלייט של בחירת עיר ללקוח
...
// הוספתי את CLI_CITY לרשימת הסטטוסים
const STATES = {
    REG_NAME: 'reg_name',
    CLN_CITY: 'cln_city',
    CLN_PRICE: 'cln_price',
    ...
    CLN_READY: 'cln_ready',
    CLI_READY: 'cli_ready',
    CLI_CITY: 'cli_city', // סטטוס חדש ללקוח
    CLI_DAY: 'cli_day',
    CLI_TIME: 'cli_time'
};
...
const Handlers = {
    ...
    async cleaner(from, profile, msg) {
        ...
        if (state === 'bid_price' && jobId) {
            ...
            if (isNaN(price) || price <= 0) return Messaging.sendMsg(from, "נא להזין מחיר תקין (מספר בלבד).");
        ...
        switch (state) {
            case STATES.CLN_CITY:
                ...
                if (msg === 'לא' || msg === 'זהו' || msg === 'no_more_cities') {
                    ...
                    return Messaging.sendMsg(from, "מצוין. מה המחיר הממוצע שלך לשעה?");
                } 
    ...
    async client(from, profile, msg) {
        ...
        if (jobAwaiting && /^\d+$/.test(msg)) {
            ...
            if (res[0].success) {
                await Messaging.sendMsg(from, `בשעה טובה! פרטי המנקה:\nטלפון: ${sel.cleaner_phone.replace('whatsapp:', '')}\nשם: ${sel.full_name}`);
                await Messaging.sendMsg(sel.cleaner_phone, `מזל טוב! נבחרת לעבודה! צרי קשר: ${from.replace('whatsapp:', '')}`);
            }
            return;
        }
        ...
        if (msg.includes('תיאום') || msg === 'תיאום מנקה') {
            ...
            // תיקון קריטי: מעבר לבחירת עיר, לא ישר ליום!
        ...
        switch (state) {
            ...
            case STATES.CLI_TIME:
                ...
                // סינון מנקיות לפי העיר שהלקוח בחר כרגע
                ...
                if (matched.length > 0) {
                    ...
                    return Messaging.sendMsg(from, `בקשתך נקלטה! שלחנו הודעה ל-${successCount} מנקיות ב${draft.city}. נעדכן בקרוב.`);
                } else {
                    return Messaging.sendMsg(from, `כרגע אין מנקיות פנויות ב${draft.city}. נסה שוב מאוחר יותר.`);
                }
        }
    }
};
...
app.listen(process.env.PORT || 3000, () => console.log('🚀 CleanMatch v22.0 Production'));
