...
const Messaging = {
    async sendMsg(to, body) {
        try { await twilioClient.messages.create({ from: CONFIG.TWILIO_NUMBER, to, body }); } catch (e) {}
    },
    async sendT(to, sid, vars = {}) {
        try { await twilioClient.messages.create({ from: CONFIG.TWILIO_NUMBER, to, contentSid: sid, contentVariables: JSON.stringify(vars) }); } catch (e) {}
    }
};
...
const Auction = {
    async closeAndNotify(jobId, clientPhone) {
        ...
        if (!bids || bids.length === 0) {
            ...
            return Messaging.sendMsg(clientPhone, 
                "😔 לא נמצאו הצעות למכרז זה.\n\n" +
                "💡 אפשרויות:\n" +
                "• נסה מועד אחר\n" +
                "• שלח 'תיאום מנקה' לניסיון נוסף"
            );
        }
...
const Handlers = {
    ...
    async cleaner(from, profile, msg) {
        // --- סטטוס מנקה (חדש!) ---
        if (msg === 'סטטוס' || msg === 'הפרופיל שלי') {
             const { data: activeBids } = await supabase.from('bids').select('*').eq('cleaner_phone', from).eq('status', 'pending');
             const statusMsg = `📊 הפרופיל שלך:\n⭐ דירוג: ${profile.rating} (${profile.rating_count} ביקורות)\n💼 עבודות: ${profile.total_jobs}\n📥 הצעות פעילות: ${activeBids?.length || 0}`;
             return Messaging.sendMsg(from, statusMsg);
        }

        // --- ביטול הצעה ---
        if (msg === 'ביטול' || msg === 'בטל') {
            const { data: cancelled } = await supabase.rpc('cancel_bid', { p_cleaner_phone: from });
            return Messaging.sendMsg(from, cancelled ? "✅ ההצעה בוטלה." : "לא נמצאה הצעה פעילה.");
        }

        // --- ביטול עבודה מאושרת (חדש!) ---
        if (msg === 'לא יכולה להגיע' || msg === 'ביטול עבודה') {
            const { data: myJob } = await supabase.from('jobs').select('*, bids!inner(*)').eq('bids.cleaner_phone', from).eq('bids.status', 'selected').eq('status', 'confirmed').single();
            if (!myJob) return Messaging.sendMsg(from, "אין עבודה מאושרת לביטול.");
            
            await supabase.from('jobs').update({ status: 'cancelled_by_cleaner' }).eq('id', myJob.id);
            await Messaging.sendMsg(myJob.client_phone, `😔 המנקה נאלצה לבטל את ההגעה.\nשלח 'תיאום מנקה' למציאת תחליף.`);
            return Messaging.sendMsg(from, "הודענו ללקוח. העבודה בוטלה.");
        }
        ...
        if (msg === 'אני פנוי/ה' && jobId) {
            ...
            return Messaging.sendMsg(from, "מעולה! מה הצעת המחיר שלך (סה\"כ) לעבודה זו?");
        }
        ...
        switch (state) {
            case STATES.CLN_CITY:
                ...
                if (msg === 'לא' || msg === 'זהו') {
                    ...
                    return Messaging.sendMsg(from, "מה המחיר הממוצע שלך לשעת ניקיון? (למשל: 80₪)");
                }
            ...
            case STATES.CLN_PRICE:
                ...
                return Messaging.sendMsg(from, "ספר/י על עצמך בקצרה (ניסיון, ציוד):");
    ...
    async client(from, profile, msg) {
        if (msg === 'איפוס') {
             ...
             return Messaging.sendMsg(from, "התהליך אופס. שלח 'תיאום מנקה' להתחלה מחדש.");
        }

        // --- ביטול עבודה ע"י לקוח (חדש!) ---
        if (msg === 'ביטול עבודה') {
            const { data: confirmedJob } = await supabase.from('jobs').select('*, bids!inner(cleaner_phone)').eq('client_phone', from).eq('status', 'confirmed').single();
            if (!confirmedJob) return Messaging.sendMsg(from, "אין עבודה מאושרת לביטול.");
            
            await supabase.from('jobs').update({ status: 'cancelled_by_client' }).eq('id', confirmedJob.id);
            await Messaging.sendMsg(confirmedJob.bids[0].cleaner_phone, `😔 הלקוח נאלץ לבטל את העבודה ב${confirmedJob.city}.\nנעדכן אותך בהזדמנויות הבאות.`);
            return Messaging.sendMsg(from, "העבודה בוטלה והודענו למנקה.");
        }
        ...
        if (jobAwaiting && /^\d+$/.test(msg)) {
            ...
            if (res[0].success) {
                ...
                await Messaging.sendMsg(sel.cleaner_phone, `נבחרת לעבודה! 🎉\nצרי קשר עם הלקוחה: ${from.replace('whatsapp:', '')}`);
        ...
        if (msg.includes('תיאום') || msg === 'תיאום מנקה') {
            ...
            if (existing) return Messaging.sendMsg(from, "יש לך תהליך פעיל. שלח 'איפוס' אם נתקעת.");
        ...
        switch (state) {
            case STATES.CLI_CITY:
                ...
                await supabase.from('profiles').update({ temp_state: STATES.CLI_ROOMS }).eq('phone_number', from);
                return Messaging.sendMsg(from, "כמה חדרים בדירה? (למשל: 3, 4, 5)");

            case STATES.CLI_ROOMS:
                await supabase.from('jobs').update({ num_rooms: msg.trim() }).eq('client_phone', from).eq('status', 'draft');
            ...
            case STATES.CLI_DAY:
                ...
                const dateDisplay = targetDate.toLocaleDateString('he-IL', { day: 'numeric', month: 'numeric' });
                return Messaging.sendMsg(from, `באיזו שעה ב${msg} (${dateDisplay})? (למשל: 09:00)`);
            ...
            case STATES.CLI_TIME:
                ...
                if (matched.length > 0) {
                    ...
                    const dateObj = new Date(draft.job_date);
                    const dateStr = dateObj.toLocaleDateString('he-IL', { weekday: 'long', day: 'numeric', month: 'numeric' });
                    const dateTimeStr = `${dateStr} ב-${msg}`;

                    await broadcastToCleaners(matched, CONFIG.TEMPLATES.JOB_OFFER, {
                        "1": draft.city,
                        "2": dateTimeStr,
                        "3": draft.num_rooms
                    });
                    return Messaging.sendMsg(from, `בקשתך נקלטה! שלחנו הודעה ל-${matched.length} מנקיות.`);
                } else {
...
app.listen(process.env.PORT || 3000, () => console.log('🚀 CleanMatch v27.0 - Final Features'));
