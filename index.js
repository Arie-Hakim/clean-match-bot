...
const app = express();
app.set('trust proxy', 1); // קריטי עבור Render ו-express-rate-limit
...
// 🟢 אבטחה: אימות Twilio עם לוגים לדיבאג
const validateTwilio = (req, res, next) => {
    console.log(`\n--- [INCOMING] --- Signature: ${req.headers['x-twilio-signature'] ? 'OK' : 'MISSING'}`);
    ...
    console.error(`❌ [Auth Error] URL Mispatch: ${url}. Check WEBHOOK_URL in Render.`);
...
const Handlers = {
    async cleaner(from, profile, msg) {
        ...
        if (profile.temp_state === STATES.BID_PRICE && jobId) {
            ...
            await Messaging.sendMsg(from, "ההצעה נשלחה! נעדכן אותך אם תבחרי.");
            ...
            if (r[0].bid_count >= CONFIG.BIDS_LIMIT) await Auction.notifyClient(jobId, (await supabase.from('jobs').select('client_phone').eq('id', jobId).single()).data.client_phone);
            return true;
        }
        if (msg === 'job_accept' || msg === 'אני פנוי/ה') {
            ...
            await Messaging.sendMsg(from, "מה הצעת המחיר שלך לעבודה זו?");
            return true;
        }
        return false;
    },
    ...
    async client(from, profile, msg, draft) {
        // 2. בחירת זוכה עם ולידציית אינדקס
        ...
        if (jobAwaiting && /^\d+$/.test(msg)) {
            ...
            if (!sel) return Messaging.sendMsg(from, `בחירה לא תקינה. בחר/י 1-${bids?.length || 0}`);
        ...
        if (draft && !draft.job_time && msg.length > 2) {
            ...
            return Messaging.sendMsg(from, "מחפשת מנקות... נעדכן אותך תוך דקות ספורות.");
        }
        return false;
    }
};
...
app.get('/cron/cleanup', async (req, res) => {
    ...
    if (expired) for (const j of expired) await Auction.notifyClient(j.id, j.client_phone);
    ...
    try {
        ...
        switch (state) {
            ...
            case STATES.CITY:
                ...
                await supabase.from('profiles').update({ city: c }).eq('phone_number', from);
            ...
            case STATES.READY:
                ...
                break;
            case STATES.DAY:
                const d = CONFIG.DAYS.indexOf(msg);
                if (d !== -1) {
                    const dt = new Date(); dt.setDate(dt.getDate() + (d + 7 - dt.getDay()) % 7);
                    await supabase.from('jobs').update({ job_date: dt.toISOString().split('T')[0] }).eq('client_phone', from).eq('status', 'draft');
                    await Messaging.sendMsg(from, "באיזו שעה?");
                }
                break;
        }
        res.status(200).send('OK');
    } catch (e) { console.error(`[Fatal]`, e); res.status(200).send('OK'); }
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log('🚀 CleanMatch v14.0 Production Live'));
